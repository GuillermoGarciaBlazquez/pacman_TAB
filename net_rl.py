...
PLOT_FILE = f"{MODEL_NAME}_metrics.png"
    ...
    for episode in range(NUM_EPISODES):
        ...
        for t in range(MAX_STEPS):
            ...
            if len(memory) >= BATCH_SIZE:
                ...
                q_values_batch = policy_net(states)
                q_values_selected = q_values_batch.gather(1, actions.unsqueeze(1)).squeeze(1)
                next_q_values = target_net(next_states).max(1)[0]
                expected_q_values = rewards + (GAMMA * next_q_values * (1 - dones))
                loss = nn.MSELoss()(q_values_selected, expected_q_values.detach())
        ...
        print(f"Episode {episode+1}/{NUM_EPISODES} | "
              ...
              f"Loss std: {std_loss:.4f} | "
              f"Max Q: {max_q:.2f} | Min Q: {min_q:.2f} | Mean Q: {mean_q:.2f} | "
